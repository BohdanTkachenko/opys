...
One `INDEX.md` at the base (grouped by type) is regenerated by
...
and linkifies prose before regenerating the index. `opys sync` rebuilds it after
hand edits. Regeneration refuses to run if any document fails to parse (run
`verify` first). Slicing the inventory is done live with `opys list`, not
